...
const AGENTS: Agent[] = [
  { id: "sasa", name: "SASA", icon: "agent-sasa.svg", iconSize: 16, chip: "bg-chip-blue", expandable: false },
  { id: "workshop", name: "Workshop Assist", icon: "agent-workshop.svg", iconSize: 16, chip: "bg-chip-amber", expandable: false },
  { id: "crco", name: "CR/CO Agent", icon: "agent-crco.svg", iconSize: 20, chip: "bg-chip-green", expandable: true },
];
...
export function AgentSidebar({ onStartCr }: { onStartCr?: () => void }) {
  ...
  const visible = query ? AGENTS.filter((a) => a.name.toLowerCase().includes(query)) : AGENTS;
  ...
  return (
    <aside
      className={cn(
        ...
        collapsed ? "w-rail border-r border-line-faint" : "w-sidebar border-r-[0.5px] border-line-soft",
      )}
    >
      {/* ── Header band ─────────────────────────────────────────────────── */}
      <div
      ...
      >
        <div className="flex h-full min-w-0 flex-1 items-center justify-between">
          {collapsed ? (
            <span className="relative flex h-full items-center overflow-clip rounded-md bg-surface px-3 py-2">
      ...
      >
        <div className={cn("flex w-full flex-col", collapsed ? "items-center" : "gap-3")}>
          {collapsed ? (
          ...
          <div className={cn("flex w-full flex-col", collapsed ? "items-center" : "gap-1")}>
            {visible.map((agent) => {
              ...
              return (
                <div key={agent.id} className="flex w-full flex-col gap-3">
                  <button
                    type="button"
                    onClick={agent.expandable ? () => toggleAgent(agent.id) : undefined}
                    ...
                    className={cn(
                      ...
                      agent.expandable ? "w-full justify-between" : "w-[13rem] justify-between",
                    )}
                  >
              ...
              <div className="flex w-[13rem] flex-col gap-2">
                ...
                >
                  <span className="flex w-full items-center justify-between">
                    ...
                    <span className={cn("block", !recentsExpanded && "-scale-y-100")}>
