 ...
 *
 * The children slot does NOT scroll — the child owns its own scrolling. The
 * transcript has to, because it decides whether to follow new turns or leave the
 * user where they scrolled to, and it cannot make that decision about a scroll
 * container it does not control. Two nested scrollers would also fight.
 */
...
export function AppShell({
  ...
  chatTitle?: string;
  onStartCr?: () => void;
  /** "New Chat" — abandons the current conversation and opens a fresh one. */
  onNewChat?: () => void;
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-linear-to-t from-canvas-from to-canvas-to">
      <IconRail />
      <AgentSidebar onStartCr={onStartCr} onNewChat={onNewChat} />
