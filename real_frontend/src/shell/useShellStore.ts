...
export const useShellStore = create<ShellState>((set) => ({
  railExpanded: false,
  /*
   * Opens expanded, as the Figma frame does.
   *
   * It briefly opened collapsed to give the conversation more width; the client
   * asked for the agent list to be visible on arrival instead. That costs 136px
   * of the reference width, which is why the root scale above was tuned in the
   * same change — the two were traded against each other deliberately.
   */
