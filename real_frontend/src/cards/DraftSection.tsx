      ...
      <div className="flex w-full flex-col gap-2 rounded-md border border-brand-a52 bg-surface px-3 py-3">
        ...
        <div className="flex items-center justify-between gap-3 border-t border-line-soft pt-2">
          <p className="min-w-0 flex-1 text-10 font-text text-ink-450">
            Sends <span className="text-ink-900">{field.label}</span> back to
            the agent, which revalidates and re-presents the draft.
          </p>
...
function DraftField({
  ...
  if (activity?.kind === "edit") {
    return (
      <FieldEditor field={field} onCancel={onCancel} onSubmit={onSubmitValue} />
    );
  }
...
export function DraftSection({
  ...
  return (
    ...
    >
      ...
      {open ? (
        /*
         * Two columns, now that an open section has the whole card width.
         *
         * A field being edited or retried spans both: the editor carries a
         * textarea or a row of allowed-value chips, and the compare view shows
         * the original and the candidate side by side. Half a card is not enough
         * for either, and a cramped editor is how people mistype a value into a
         * change request.
         */
        <div className="grid w-full grid-cols-1 gap-2 md:grid-cols-2">
          {fields.map((field) => (
            <div
              key={field.key}
              className={cn(
                activeKey === field.key && activity && "md:col-span-2",
              )}
            >
              <DraftField
                field={field}
                ...
                // one answer.
                disabled={
                  disabled || (activeKey !== null && activeKey !== field.key)
                }
          ...
          {fields.length === 0 ? (
            <p className="text-12 font-text text-ink-450">
