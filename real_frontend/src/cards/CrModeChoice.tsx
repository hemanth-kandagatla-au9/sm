...
export function CrModeChoice({ props, respond, pending, answer }: AgentCardProps<"crModeChoice">) {
  ...
  // Seeded from the settled answer so a restored transcript shows what was
  // chosen. Local state alone does not survive a reload.
      ...
      <div className="flex w-full flex-col gap-5" role="radiogroup" aria-label={props.title}>
        {props.modes.map((mode) => (
          <button
            ...
            disabled={!pending}
            onClick={() => choose(mode.value, mode.label)}
            className={cn(
