...
export async function POST(req: NextRequest) {
  /*
   * A scripted backend must never exist in a production build. Without this,
   * `/api/agent/mock` would ship as a live endpoint that answers agent traffic
   * with fixtures — a route that fabricates approval screens is not something to
   * leave reachable, and "nothing links to it" is not a control.
   *
   * The dev-only pages are guarded the same way; this one matters more, because
   * it produces data rather than displaying it.
   */
